  ...
  useEffect(() => {
    if (selection) {
      ...
      if ("lat" in selection && "lng" in selection) {
        map.panTo({
          lat: selection.lat as number,
          lng: selection.lng as number,
        });
      }
    }
  }, [selection]);
  ...
  const ensure_visible = (element_id: number) => {
    ...
    if (element_to_show) {
      ...
      if (scrolling_parent) {
        ...
        var scroll_by = 0;
        if (now_top < top) scroll_by = -(top - now_top);
        else if (now_bot > bot) scroll_by = now_bot - bot;
        if (scroll_by != 0) {
            ...
            {locationList.features.map((vitri, index) => {
              ...
              return (
                <Marker
                  key={index}
                  position={{
                    lat: vitri.lat as number,
                    lng: vitri.lng as number,
                  }}
                  title={vitri.address}
                  //onLoad={map=>setMap(map)}
                  onClick={() => {
...
const PlacesAutocomplete = ({
  ...
  useEffect(() => {
    if (pathname && pathname !== "/locations") {
      ...
      console.log("storeId", lastSegment);
  ...
  const handleSearch = () => {
    const filteredList = Data.CategoryListData.features.filter((location) =>
      location.storeId.toLowerCase().includes(result)
    );
    setLocationList({ ...Data.CategoryListData, features: filteredList });
  };
        ...
        arrDistance.push(
          calculateDistance(
            ...
            Object.values(currentLocation)[0]
              ? Object.values(currentLocation)[0]
              : Object.values(userLocation)[0],
            Object.values(currentLocation)[1]
              ? Object.values(currentLocation)[1]
              : Object.values(userLocation)[1]
          )
        )
      );
    }
  ...
  const Filter = (check: keyof (typeof Data.CategoryListData.features)[0]) => {
    const filteredData = Data.CategoryListData.features.filter(
      (item) => item[check] !== ""
    );
  ...
  return (
    <div className="autocompleteWrapper relative">
      ...
      {showFilter && (
        <div className="absolute top-0 left-0 max-md:top-5 w-full h-full bg-white border-[1px] border-gray-300 shadow-xl rounded-lg z-40">
          <div className="w-full h-full relative p-5">
            ...
            <p className="text-[22px] max-md:text-[18px] text-[#444444] font-semibold">
              Amenities
            </p>
            <div className="w-full py-5 max-md:py-3 px-5 max-md:px-3 md:flex justify-between max-md:text-[14px]">
              <p
                onClick={() => Filter("serviceShop")}
                className="cursor-pointer hover:font-semibold hover:text-[#0063A9]"
              >
                Service Shop
              </p>
              <p
                onClick={() => Filter("carwash")}
                className="cursor-pointer hover:font-semibold hover:text-[#0063A9] max-md:pt-2"
              >
                Car Wash
              </p>
              <p
                onClick={() => Filter("amazonLocker")}
                className="cursor-pointer hover:font-semibold hover:text-[#0063A9]  max-md:pt-2"
              >
                Amazon Locker
              </p>
            ...
            <p className="text-[22px] text-[#444444] max-md:text-[18px] font-semibold pt-5">
            ...
            <div className="w-full py-5 max-md:py-3 px-5 max-md:px-3 md:flex justify-between max-md:text-[14px]">
              ...
              <p
                onClick={() => Filter("diesel")}
                className="cursor-pointer hover:font-semibold hover:text-[#0063A9] max-md:pt-2"
              >
                Diesel
              </p>
              <p
                onClick={() => Filter("propane")}
                className="cursor-pointer hover:font-semibold hover:text-[#0063A9] max-md:pt-2"
              >
                Propane
              </p>
              <p
                onClick={() => Filter("alcohol")}
                className="cursor-pointer hover:font-semibold hover:text-[#0063A9] max-md:pt-2"
              >
                Alcohol
              </p>
      ...
      <fieldset>
        <legend className="py-4 max-md:text-[14px]">
          Find a PowerMarket Near You
        </legend>
          ...
          <div className="flex gap-5 pb-3 ">
            <label className="flex cursor-pointer gap-2 text-size-4 font-bold leading-6 max-md:text-[14px]">
              <input
                type="radio"
                name="searchMode"
                className="h-6 w-6 cursor-pointer accent-brand-red max-md:text-[14px]"
                value="location"
                checked={radioSearch === "location"}
                onChange={onOptionChange}
              />
              <span className="sr-only ">Search </span>By Location
            </label>
            <label className="flex cursor-pointer gap-2 text-size-4 font-bold leading-6 max-md:text-[14px]">
              <input
                type="radio"
                name="searchMode"
                className="h-6 w-6 cursor-pointer accent-brand-red"
                value="storeId"
                checked={radioSearch === "storeId"}
                onChange={() => {
                  handleSearch(), setRadioSearch("storeId");
                }}
              />
              <span className="sr-only">Search </span>By Store #
            </label>
          </div>
          <div
            onClick={togglefilter}
            className="pr-3 pb-3 flex gap-3 items-center cursor-pointer hover:text-[#0063A9] hover:font-semibold"
          >
            <IoFilter />
            Filter
          </div>
      ...
      <div className="store-locator_search-pane-container">
        <div
          className="grid mg-top store-locator_store-search-pane"
          id="store-locator_store-search-pane"
        >
              ...
              >
                <a>
                  <Link
                    ...
                    className="store-list-card_root hover:shadow-xl"
                    id={"store-list-card_root-" + index}
                  >
                    <div className="store-list-card_content max-md:text-[14px]">
                      ...
                      <div className="flex flex-wrap gap-5">
                        {item.diesel !== "" && <p>DIESEL</p>}
                        {item.carwash !== "" && <p>CAR WASH</p>}
                        {item.propane !== "" && <p>PROPANE</p>}
                        {item.deliveryService !== "" && <p>DELIVERY</p>}
                        {item.serviceShop !== "" && <p>SERVICE SHOP</p>}
                        {item.amazonLocker !== "" && <p>AMAZON LOCKER</p>}
                        {item.alcohol !== "" && <p>ALCOHL</p>}
                      </div>
