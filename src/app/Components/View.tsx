  ...
  return (
    <div className="w-full flex justify-center py-[60px] max-lg:py-10 max-md:py-6">
      <div className="w-[90%] max-w-[880px] relative">
        <div className="w-full text-center pb-8 text-[#444444] text-[28px] font-semibold">
          Take A Look Around
        </div>
        ...
        <div className="w-full pt-6 flex justify-between max-md:hidden">
          {data.map((item) => (
            <div
              key={item.id}
              className="w-[15%] h-[125px] rounded-[16px] cursor-pointer transition-transform transform hover:scale-105"
            >
              <Image
                ...
                className={`w-full h-full object-cover rounded-[16px] ${
                  item.id === select.id && "border-[5px] border-[#75BE41]"
                }`}
              ></Image>
            </div>
          ))}
        ...
        <div className="w-full mx-auto mt-8 relative md:hidden">
          ...
          >
            {data.map((item) => (
              <button key={item.id} className="w-full h-[300px]">
                <Image
                  src={item.image}
                  alt={`image ${item.id}`}
